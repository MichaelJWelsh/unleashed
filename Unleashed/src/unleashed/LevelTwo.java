...
@Info(
	...
	purpose = "Is level TWO. Play through this to move on to next level"
	)
...
public class LevelTwo extends BasicGameState{
	...
	public void init(GameContainer gc, StateBasedGame sbg) throws SlickException {
		...
		terrain = new Terrain(player, "world/Level Two.txt");
	...
	public void update(GameContainer gc, StateBasedGame sbg, int delta) throws SlickException {
		...
		if(input.isKeyPressed(Game.keybindEscape)){
			...
			Player_Movement:
				...
				if(terrain.getTerrainSet().iterator().next().getTranslatedDistanceHorizontal() < -5972){
					Game.score.addToScore(ID, timer.getTimeRemaining());
					sbg.enterState(Game.LEVEL_THREE);
				}
